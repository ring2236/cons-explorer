...
CoNS Explorer 是一个用于理解因果图、结构因果模型和常见因果推断偏差的交互式前端。它把五个不同专业领域的因果模型呈现为可操作的有向图，允许用户同时对多个节点设置离散值，并即时观察这些设置如何沿着因果关系传播，最终改变一个或多个结果节点。
...
- 360 个预先计算的静态情景，其中 355 个包含至少一次主动设置；
...
1. 选择 ICU、水稻、货币政策、教育或供应链中的一个数据集。
...
每个数据集只有一个“全部自然”情景，所以 360 个情景中共有 `360 − 5 = 355` 个主动设置情景。

一个情景通过稳定的 key 唯一标识，例如：

```text
icu_septic_shock|N=0.2|M=natural
...
|---|---|---:|---:|---|---:|---|
| ICU 脓毒症休克 | 重症医学 | 9 | 13 | 去甲肾上腺素剂量、平均动脉压 | 36 | 急性肾损伤风险、收入 ICU 概率 |
| 水稻氮管理 | 农学 | 9 | 15 | 施氮量、灌溉量 | 36 | 水稻产量 |
| 货币政策传导 | 宏观经济 | 8 | 12 | 政策利率、银行贷款利率 | 36 | 核心通胀率、失业率 |
...
当前 v3 叙事由 `deepseek-v4-flash` 在本地离线批量生成，共进行 360 次单情景请求。v3 不再追求完整短篇小说式的人物塑造与环境描写，而要求每句话服务于因果理解。生成完成后写入 `lib/narratives-v3.generated.json`，并通过完整边覆盖、数量、key 和最低长度校验。API Key 只在生成时由本地终端读取，不进入浏览器代码，也不需要配置到 Cloudflare。
...
→ 从 narratives-v3.generated.json 读取两种解释
→ 即时更新图、对照表和文本
```
...
| `lib/scenarios.generated.json` | 360 个预先计算的情景结果 |
| `lib/narratives-v3.generated.json` | 360 个专业说明与情境化因果故事 |
...
→ 离线生成 narratives-v3.generated.json
→ 前端构建
→ Cloudflare 或单文件离线包
```
...
源材料中的部分相关系数来自加入噪声后的蒙特卡洛模拟，用于验证偏差结构确实可能出现；前端的 360 个情景则来自无噪声结构方程的确定性求值。两者用途不同，不应混为一谈。
